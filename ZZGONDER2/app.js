        ...
        // Sadece fotoğraf varsa =>
            ...
            getMaxCount().then((maxCount) => {
                ...
                db.run(
                    ...
                    [currentUsername, message, getCurrentDate(), getCurrentTime(), ip, `https://umutsar.net/UsersPhotos/${path.basename(req.file.path)}`, maxCount + 1],
                    function (err) {
                        if (err) {
                            return console.log(err.message);
                        }
                        getToPersonSessionId().then((result) => {
                            ...
                            db.get("SELECT sessionId FROM kullanicilar WHERE kullaniciadi = ?;", [req.session.username], function (err, row) {
                                ...
                                { count: lastCount + 1, username: currentUsername, message: message, time: getCurrentTime(), date: getCurrentDate(), photoPath: `https://umutsar.net/UsersPhotos/${path.basename(req.file.path)}`, myMessage: false });
                                
                                
                                if (result != row.sessionId) {

                                    io.to(row.sessionId).emit("message", 
                                    { count: lastCount + 1, username: currentUsername, message: message, time: getCurrentTime(), date: getCurrentDate(), photoPath: `https://umutsar.net/UsersPhotos/${path.basename(req.file.path)}`, myMessage: true});
            ...
            db.all('SELECT subDatabase FROM bildirimlertablosu WHERE user = ?', [req.body.toPerson], (err, row) => {
                ...
                if (row) {
                    ...
                    console.log("row: ", row)
                    console.log("row.subDatabase: ", row.subDatabase)
                    try {
                ...
                db.run(
                    ...
                    function (err) {
                        ...
                        getToPersonSessionId().then((result) => {
                            ...
                            db.get("SELECT sessionId FROM kullanicilar WHERE kullaniciadi = ?;", [req.session.username], function (err, row) {
                                io.to(result).emit("message", { count: lastCount + 1, username: currentUsername, message: message, time: getCurrentTime(), date: getCurrentDate(), photoPath: `https://umutsar.net/UsersPhotos/${path.basename(req.file.path)}` });
                                if (result != row.sessionId) {
                                    io.to(row.sessionId).emit("message", { count: lastCount + 1, username: currentUsername, message: message, time: getCurrentTime(), date: getCurrentDate(), photoPath: `https://umutsar.net/UsersPhotos/${path.basename(req.file.path)}` });
                                }
                            })
                        })
                    }
